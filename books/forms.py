...
class AddProductForm(forms.ModelForm):
    Seller_phoneno = forms.IntegerField(label="phoneno", widget=forms.TextInput(
        attrs={"placeholder": "phoneno", 'class': 'form-control', 'style': 'margin-bottom:10px; width:450px'}), error_messages={'required': 'phoneno is required'})
#  validators=[validate_Seller_phoneno],

    class Meta:
        model = UploadsBook

        fields = ['isbn', 'Book_Name', 'Seller_phoneno', 'Seller_address', 'Author_Name', 'ReleaseDate',
                  'Selling_price', 'Description', 'Publication', 'Types_of_Book', 'Label', 'Quantity', 'Image']
